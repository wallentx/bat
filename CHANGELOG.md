...
- Added `-t`/`--tokens`/`--style=tokens` and `-T`/`--tokens-wide`/`--style=tokens_wide` options to output tokenized lines with semantic scopes, similar to `pygmentize -f tokens`. PR #1 (@wallentx)
